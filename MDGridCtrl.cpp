...
CMDGridCtrl::CMDGridCtrl()
{
	m_font.CreatePointFont(120,"宋体");

	for(int i=0; i<MD_GRID_ROW; i++)
	{
		for(int j=0; j<MD_GRID_COL; j++)
		{
			m_arrData[i][j] = 0;
		}
	}	
...
BEGIN_MESSAGE_MAP(CMDGridCtrl, CWnd)
	//{{AFX_MSG_MAP(CMDGridCtrl)
	ON_WM_PAINT()
	ON_WM_SIZE()
	ON_WM_LBUTTONDOWN()
	ON_WM_RBUTTONDOWN()
...
/////////////////////////////////////////////////////////////////////////////
// CMDGridCtrl message handlers

void CMDGridCtrl::OnPaint() 
{
...
void CMDGridCtrl::DrawLine(CDC *pDC)
{
	CRect rcClient;
	GetClientRect(&rcClient);
	
	int nItemWidth = rcClient.Width() / (MD_GRID_COL+1);
	int nItemHeight = rcClient.Height() / MD_GRID_ROW;

	int nHeight = 0;
	int i = 0;
	for(i = 0; i < MD_GRID_ROW +1; i++)
	{
...
void CMDGridCtrl::OnSize(UINT nType, int cx, int cy) 
{
	CWnd::OnSize(nType, cx, cy);
	CRect rcClient;
	GetClientRect(&rcClient);

	int nWndWidth = rcClient.Width() - (rcClient.Width() % (MD_GRID_COL+1));
	int nWndHeight = rcClient.Height() - (rcClient.Height() % MD_GRID_ROW);	
	
	SetWindowPos(NULL, 0, 0, nWndWidth+1, nWndHeight+1, SWP_NOMOVE);
}

void CMDGridCtrl::DrawText(CDC *pDC)
{
	CRect rcClient;
	GetClientRect(&rcClient);
	int nItemWidth = rcClient.Width() / (MD_GRID_COL+1);
...
void CMDGridCtrl::DrawBlock(CDC *pDC)
{
	CRect rcClient;
	GetClientRect(&rcClient);
...
void CMDGridCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	m_ptStart = m_ptStop = point;
	CWnd::OnLButtonDown(nFlags, point);
}

void CMDGridCtrl::SelectAll()
{
	NotifyChange();
...
void CMDGridCtrl::Select(CPoint point)
{
	NotifyChange();
	for(int i=0; i<MD_GRID_ROW; i++)
		for(int j=0; j<MD_GRID_COL; j++)
...
void CMDGridCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{	
	m_ptStart = m_ptStop = point;
	SelectOneRow(point);
...
void CMDGridCtrl::SelectOneRow(CPoint point)
{
	NotifyChange();
...
void CMDGridCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if(m_bDrag)
	{
		ClipCursor(NULL);
...
void CMDGridCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{	
...
void CMDGridCtrl::DrawBk(CDC *pDC)
{
	CRect rcClient;
	GetClientRect(&rcClient);
...
void CMDGridCtrl::OnRButtonUp(UINT nFlags, CPoint point) 
{
...
BOOL CMDGridCtrl::IsInClient(CPoint point)
{
	CRect rcClient;
	GetClientRect(&rcClient);
	int nItemWidth = rcClient.Width() / (MD_GRID_COL+1);
...
void CMDGridCtrl::SetWeekText(const CStringArray& arr)
{
...
void CMDGridCtrl::SetAllValue(int nVal)
{
	NotifyChange();
...
int CMDGridCtrl::GetGridItemData(int x, int y)
{
	return m_arrData[x][y];
}

void CMDGridCtrl::SetGridItemData(int x, int y, int nVal)
{
	NotifyChange();
...
void CMDGridCtrl::PreSubclassWindow() 
{
	CRect rcClient;
	GetClientRect(&rcClient);
	int nWndWidth = rcClient.Width() - (rcClient.Width() % (MD_GRID_COL+1));
...
void CMDGridCtrl::ChangeSelectAllState()
{
	if(!IsAllSelect())
	{
		if(m_arrData[0][0] != 0)
